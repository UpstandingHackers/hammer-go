...
func init_parser() {
	...
	var bsfdig_4bit hammer.HParser = hammer.In("AEIMQUYcgkosw048", 16)
	var bsfdig_2bit hammer.HParser = hammer.In("AQgw", 4)
...
func main() {
	...
	// show what we got
	inputsize = len(input)
	fmt.Printf("inputsize=%lu\ninput=", inputsize)

	// parse input
	result = hammer.Parse(document, input, inputsize)

	// show result of parse actions
	if result {
		fmt.Printf("parsed=%lld bytes\n", result.bit_length / 8)
